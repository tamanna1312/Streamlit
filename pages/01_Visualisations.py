		...
		with tab2:
			...
			for j, col in zip(range(len(folders)), colours):
					...
					if select_normalising=='CI':
						...
						st.write(a)
						st.write(CI_Norm1)
						plt.plot(CI_Norm1[CI_Norm1 != 0], c = col, label = folders[j])
					...
					if select_normalising=='CH':
						...
						st.write(CH_Norm)
						CH_Norm1 = CH_Norm.T
						plt.plot(CH_Norm1[CH_Norm1 != 0], c = col, label = folders[j])
					...
					if select_normalising=='CM':
						...
						CM_Norm1 = CM_Norm.T
						plt.plot(CM_Norm1[CM_Norm1 != 0], c = col, label = folders[j])
